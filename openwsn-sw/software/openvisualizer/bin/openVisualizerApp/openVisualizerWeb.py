...
if __name__=="__main__":
    ...
    # Run OpenWSN by default Sink : Mote 1 !! added by erfan!!
    if (app.simulatorMode == True):
        time.sleep(3) #Give me a moment to load!
        webapp._toggleDAGroot('0001')

    # added by erfan to check end time : if time.time() > future:
    #now = time.time()
    #future = now + 60
    while True:
     #   if time.time() > future:
      #      print '\n Timer reached to its limit(set to 60 seconds)!! ((openVisualizerweb.py #L=748))'
       #     app.close()
