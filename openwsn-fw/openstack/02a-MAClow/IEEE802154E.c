...
port_INLINE uint8_t calculateFrequency(uint8_t channelOffset) {
   ...
   //return SYNCHRONIZING_CHANNEL; // single channel  								// commented by erfan !!!
   return 11+(ieee154e_vars.asnOffset+channelOffset)%16; //channel hopping			// uncommented by erfan !!!
}
...
void endSlot() {
   ...
   // added by erfan to calculate duty cycle
   open_addr_t *        p;
   ...
   FILE *GetDutyCycle;
   GetDutyCycle =fopen("_Log_DutyCycle.txt","a");
   
   if (GetDutyCycle != NULL) {
      fprintf(GetDutyCycle, "%d %02d %02d %02d %0d %.2f\n", p->addr_64b[7], t->tm_hour, t->tm_min, t->tm_sec, tp.tv_usec/1000, ieee154e_stats.numTicsOn/(float)ieee154e_stats.numTicsTotal);
   }
   fclose(GetDutyCycle);
   // end of erfan
