   ...
   if (idmanager_getIsDAGroot()==FALSE ||
      ...
      forwarding_receive(
       ...
       if(msg->l4_protocol == IANA_UDP){
           ...
           uint8_t i;
           FILE *GetPDRandDELAY;
           GetPDRandDELAY =fopen("_Log_Iphc_Rcv.txt","a");

           if(GetPDRandDELAY != NULL){
               fprintf(GetPDRandDELAY, "%d %02d %02d %02d %d\n", ipv6_header.src.addr_128b[15], t->tm_hour, t->tm_min, t->tm_sec, tp.tv_usec/1000);
               for(i=0;i<130;i++)
                   fprintf(GetPDRandDELAY, "%d ", msg->packet[i]);
