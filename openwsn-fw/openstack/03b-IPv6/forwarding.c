...
void forwarding_receive(
   ...
   if (
      ...
      // added by erfan : to get the queuing delay of fw packets, compared with logged fw pkts in openqueue
      uint8_t l4protocol;
      if (ipv6_header->next_header==IANA_IPv6HOPOPT) {
	      l4protocol = ipv6_hop_header->nextHeader;
      }
      else {
	      l4protocol = ipv6_header->next_header;
      }
      if ( l4protocol == IANA_UDP ) {
	      struct tm * t;
	      struct timeval tp;
	      gettimeofday(&tp, 0);
	      ...
	      uint8_t j;
	      FILE *Delayfw;
	      Delayfw =fopen("_Log_forwardinglevel.txt","a");
	      open_addr_t *        p;
	      p=idmanager_getMyID(ADDR_64B);
	      if(Delayfw != NULL){                                // current mote   // time of sending pkts to queue                           
		  fprintf(Delayfw, "%d %02d %02d %02d %d %d %d\n", p->addr_64b[7], t->tm_hour, t->tm_min, t->tm_sec, tp.tv_usec/1000, ipv6_header->src.addr_128b[15], ipv6_header->dest.addr_128b[15]);
		  for (j=0;j<130;j++) {
		       fprintf(Delayfw, "%d ", msg->packet[j]);// pkt's data
		  }
		  fprintf(Delayfw, "\n");
	      }
	      fclose(Delayfw);
